...
//#include "menu.h"
#define RET_CHK(x) if (DX11::InputHook::bBlockInput && DX11::InputHook::showMenu) { return x; }\
...
namespace DX11
{
	...
	bool InputHook::showMenu = true;

	void InputHook::ValidateHook(HWND hWindow)
	{
		if (DX11::InputHook::showMenu && !bHookLoaded) {
			LoadHook(hWindow);
		}
		else if (!DX11::InputHook::showMenu && bHookLoaded && bUnhookWhenHidden) {
			UnloadHook(hWindow);
		}
	}
